...
describe Puppet::Type.type(:pe_postgresql_psql), "when validating attributes" do
  [:name, :unless, :db, :psql_path, :psql_user, :psql_group].each do |attr|
    it "should have a #{attr} parameter" do
      expect(Puppet::Type.type(:pe_postgresql_psql).attrtype(attr)).to eq(:param)
    end
  end

  [:command].each do |attr|
    it "should have a #{attr} property" do
      expect(Puppet::Type.type(:pe_postgresql_psql).attrtype(attr)).to eq(:property)
    end
  end
end

describe Puppet::Type.type(:pe_postgresql_psql), :unless => Puppet.features.microsoft_windows? do
  subject do
    Puppet::Type.type(:pe_postgresql_psql).new({:name => 'rspec'}.merge attributes)
  end
