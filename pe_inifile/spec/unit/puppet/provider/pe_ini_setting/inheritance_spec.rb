...
# documented
$: << 'spec/fixtures/modules/inherit_pe_ini_setting/lib'
provider_class = Puppet::Type.type(:inherit_pe_ini_setting).provider(:pe_ini_setting)
describe provider_class do
  include PuppetlabsSpec::Files

  let(:tmpfile) { tmpfilename('inherit_pe_ini_setting_test') }
...
green = purple
  ...
  context 'when ensuring that a setting is present' do
    ...
    it 'should add a value to the file' do
      provider_class.stubs(:file_path).returns(tmpfile)
      resource = Puppet::Type::Inherit_pe_ini_setting.new({
        :setting => 'set_this',
        :value   => 'to_that',
      })
