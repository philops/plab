...
provider_class = Puppet::Type.type(:pe_ini_setting).provider(:ruby)
describe provider_class do
  include PuppetlabsSpec::Files

  let(:tmpfile) { tmpfilename("pe_ini_setting_test") }
  let(:emptyfile) { tmpfilename("pe_ini_setting_test_empty") }

  let(:common_params) { {
      :title    => 'pe_ini_setting_ensure_present_test',
      :path     => tmpfile,
      :section  => 'section2',
  } }
  ...
  context 'when calling instances' do
    ...
    it 'should fail when file path is not set' do
      ...
      }.to raise_error(Puppet::Error, 'Pe_ini_settings only support collecting instances when a file path is hard coded')
    end
    ...
    context 'when file path is set by a child class' do
      ...
      it 'should override the provider instances file_path' do
        ...
        resource = Puppet::Type::Pe_ini_setting.new(common_params)
...
subby=bar
 ...
 ; yet another comment
    ...
    it "should add a missing setting to the correct section" do
      resource = Puppet::Type::Pe_ini_setting.new(common_params.merge(
...
foo=bar
    ...
    it "should fail if the separator doesn't include an equals sign" do
      expect {
        Puppet::Type::Pe_ini_setting.new(common_params.merge(
                                         :section           => 'section2',
                                         :setting           => 'foo',
                                         :value             => 'yippee',
                                         :key_val_separator => '+'))
...
[section:sub]
 ...
 ; yet another comment
    ...
    it "should update an existing setting at the correct indentation when the header is aligned" do
      resource = Puppet::Type::Pe_ini_setting.new(
