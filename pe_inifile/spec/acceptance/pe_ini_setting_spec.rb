...
describe 'pe_ini_setting resource' do
  after :all do
  ...
  describe 'ensure parameter' do
    context '=> present for global and section' do
      pp = <<-EOS
      pe_ini_setting { 'ensure => present for section':
        ensure  => present,
        path    => "#{tmpdir}/pe_ini_setting.ini",
        section => 'one',
        setting => 'two',
        value   => 'three',
      }
      pe_ini_setting { 'ensure => present for global':
        ensure  => present,
        path    => "#{tmpdir}/pe_ini_setting.ini",
        section => '',
        setting => 'four',
      ...
      describe file("#{tmpdir}/pe_ini_setting.ini") do
        it { should be_file }
        #XXX Solaris 10 doesn't support multi-line grep
    ...
    context '=> absent for key/value' do
      before :all do
        if fact('osfamily') == 'Darwin'
          shell("echo \"four = five\n[one]\ntwo = three\" > #{tmpdir}/pe_ini_setting.ini")
        else
      ...
      pp = <<-EOS
      pe_ini_setting { 'ensure => absent for key/value':
        ensure  => absent,
        path    => "#{tmpdir}/pe_ini_setting.ini",
        section => 'one',
        setting => 'two',
    ...
    context '=> absent for section', :pending => "cannot ensure absent on a section"  do
      ...
      after :all do
        shell("cat #{tmpdir}/pe_ini_setting.ini", :acceptable_exit_codes => [0,1,2])
        shell("rm #{tmpdir}/pe_ini_setting.ini", :acceptable_exit_codes => [0,1,2])
      end

      pp = <<-EOS
      pe_ini_setting { 'ensure => absent for section':
        ensure  => absent,
        path    => "#{tmpdir}/pe_ini_setting.ini",
        section => 'one',
      }
    ...
    context '=> absent for global' do
      ...
      pp = <<-EOS
      pe_ini_setting { 'ensure => absent for global':
        ensure  => absent,
        path    => "#{tmpdir}/pe_ini_setting.ini",
        section => '',
        setting => 'four',
  ...
  describe 'section, setting, value parameters' do
    {
      ...
      context parameter_list do
        ...
        it_behaves_like 'has_content', "#{tmpdir}/pe_ini_setting.ini", pp, content
      end
    end

    {
      ...
      context parameter_list, :pending => 'no error checking yet' do
        ...
        it_behaves_like 'has_error', "#{tmpdir}/pe_ini_setting.ini", pp, error
      end
    end
  end
  ...
  describe 'key_val_separator parameter' do
    {
      ...
      context "with \"#{parameter}\" makes \"#{content}\"" do
        pp = <<-EOS
        pe_ini_setting { "with #{parameter} makes #{content}":
          ensure  => present,
          section => 'one',
          setting => 'two',
          value   => 'three',
...

    {
      ...
      context "with \"#{parameter}\" raises \"#{error}\"" do
        pp = <<-EOS
        pe_ini_setting { "with #{parameter} raises #{error}":
          ensure  => present,
          section => 'one',
          setting => 'two',
          value   => 'three',
