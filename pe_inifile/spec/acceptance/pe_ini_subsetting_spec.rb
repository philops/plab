...
describe 'pe_ini_subsetting resource' do
  after :all do
  ...
  describe 'ensure, section, setting, subsetting, & value parameters' do
    context '=> present with subsections' do
      pp = <<-EOS
      pe_ini_subsetting { 'ensure => present for alpha':
        ensure     => present,
        path       => "#{tmpdir}/pe_ini_subsetting.ini",
        section    => 'one',
        setting    => 'key',
        subsetting => 'alpha',
        value      => 'bet',
      }
      pe_ini_subsetting { 'ensure => present for beta':
        ensure     => present,
        path       => "#{tmpdir}/pe_ini_subsetting.ini",
        section    => 'one',
        setting    => 'key',
      ...
      describe file("#{tmpdir}/pe_ini_subsetting.ini") do
        it { should be_file }
    ...
    context 'ensure => absent' do
      before :all do
        if fact('osfamily') == 'Darwin'
          shell("echo \"[one]\nkey = alphabet betatrons\" > #{tmpdir}/pe_ini_subsetting.ini")
        else
          shell("echo -e \"[one]\nkey = alphabet betatrons\" > #{tmpdir}/pe_ini_subsetting.ini")
        end
      end
      ...
      pp = <<-EOS
      pe_ini_subsetting { 'ensure => absent for subsetting':
        ensure     => absent,
        path       => "#{tmpdir}/pe_ini_subsetting.ini",
        section    => 'one',
        setting    => 'key',
  ...
  describe 'subsetting_separator' do
    {
      ...
      context "with \"#{parameter}\" makes \"#{content}\"" do
        pp = <<-EOS
        pe_ini_subsetting { "with #{parameter} makes #{content}":
          ensure     => present,
          section    => 'one',
          setting    => 'two',
          subsetting => 'twine',
  ...
  describe 'quote_char' do
    {
      ...
      context %Q{with '#{parameter.first}' #{parameter.length > 1 ? '=> \'' << parameter[1] << '\'' : 'absent'} makes '#{content}'} do
        path = File.join(tmpdir, 'pe_ini_subsetting.ini')
        ...
        pp = <<-EOS
        pe_ini_subsetting { '#{parameter.first}':
