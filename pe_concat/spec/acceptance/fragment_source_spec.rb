...
describe 'pe_concat::fragment source' do
  ...
  context 'should create files containing first match only.' do
    pp = <<-EOS
      ...
      pe_concat { '#{basedir}/result_file1':
        owner   => '#{username}',
        group   => '#{groupname}',
        mode    => '0644',
      }
      pe_concat { '#{basedir}/result_file2':
        owner   => '#{username}',
        group   => '#{groupname}',
        mode    => '0644',
      }
      pe_concat { '#{basedir}/result_file3':
        owner   => '#{username}',
        group   => '#{groupname}',
        mode    => '0644',
      }
  ...
  context 'should fail if no match on source.' do
    pp = <<-EOS
      pe_concat { '#{basedir}/fail_no_source':
        owner   => '#{username}',
        group   => '#{groupname}',
        mode    => '0644',
      }
