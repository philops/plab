...
describe 'pe_concat', :type => :define do

  shared_examples 'pe_concat' do |title, params, id| 
    ...
    concatdir            = '/var/lib/puppet/pe_concat'
    ...
    let(:title) { title }
    let(:params) { params }
    let(:facts) {{ :pe_concat_basedir => concatdir, :id => id }}
    ...
    if p[:ensure] == 'present'
      ...
      it do
        should contain_exec("pe_concat_#{title}").with({
  ...
  context 'title' do
    context 'without path param' do
      ...
      ['/foo', '/foo/bar', '/foo/bar/baz'].each do |title|
        context title do
          it_behaves_like 'pe_concat', '/etc/foo.bar'
        end
      end
    ...
    context 'with path param' do
      ['./foo', 'foo', 'foo/bar'].each do |title|
        context title do
          it_behaves_like 'pe_concat', title, { :path => '/etc/foo.bar' }
        end
      end
    end
  end # title =>
  ...
  context 'as non-root user' do
    it_behaves_like 'pe_concat', '/etc/foo.bar', {}, 'bob'
  end

  context 'ensure =>' do
    ['present', 'absent'].each do |ens|
      context ens do
        it_behaves_like 'pe_concat', '/etc/foo.bar', { :ensure => ens }
      end
    end
  ...
  context 'path =>' do
    context '/foo' do
      it_behaves_like 'pe_concat', '/etc/foo.bar', { :path => '/foo' }
    end
  ...
  context 'owner =>' do
    context 'apenney' do
      it_behaves_like 'pe_concat', '/etc/foo.bar', { :owner => 'apenny' }
    end
  ...
  context 'group =>' do
    context 'apenney' do
      it_behaves_like 'pe_concat', '/etc/foo.bar', { :group => 'apenny' }
    end
  ...
  context 'mode =>' do
    context '1755' do
      it_behaves_like 'pe_concat', '/etc/foo.bar', { :mode => '1755' }
    end
  ...
  context 'warn =>' do
    [true, false, '# foo'].each do |warn|
      context warn do
        it_behaves_like 'pe_concat', '/etc/foo.bar', { :warn => warn }
      end
    end
  ...
  context 'force =>' do
    [true, false].each do |force|
      context force do
        it_behaves_like 'pe_concat', '/etc/foo.bar', { :force => force }
      end
    end
  ...
  context 'backup =>' do
    context 'reverse' do
      it_behaves_like 'pe_concat', '/etc/foo.bar', { :backup => 'reverse' }
    end

    context 'false' do
      it_behaves_like 'pe_concat', '/etc/foo.bar', { :backup => false }
    end

    context 'true' do
      it_behaves_like 'pe_concat', '/etc/foo.bar', { :backup => true }
    end
  ...
  context 'replace =>' do
    [true, false].each do |replace|
      context replace do
        it_behaves_like 'pe_concat', '/etc/foo.bar', { :replace => replace }
      end
    end
  ...
  context 'order =>' do
    ['alpha', 'numeric'].each do |order|
      context order do
        it_behaves_like 'pe_concat', '/etc/foo.bar', { :order => order }
      end
    end
  ...
  context 'ensure_newline =>' do
    [true, false].each do |ensure_newline|
      context 'true' do
        it_behaves_like 'pe_concat', '/etc/foo.bar', { :ensure_newline => ensure_newline}
      end
    end
  ...
  describe 'deprecated parameter' do
    context 'gnu =>' do
      context 'foo' do
        it_behaves_like 'pe_concat', '/etc/foo.bar', { :gnu => 'foo'}
