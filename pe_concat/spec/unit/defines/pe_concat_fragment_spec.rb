...
describe 'pe_concat::fragment', :type => :define do
  ...
  shared_examples 'fragment' do |title, params|
    ...
    it do
      should contain_class('pe_concat::setup')
      should contain_pe_concat(p[:target])
      should contain_file("#{fragdir}/fragments/#{p[:order]}_#{safe_name}").with({
        ...
        :source  => p[:source],
        :content => p[:content],
        :alias   => "pe_concat_fragment_#{title}",
        :backup  => false,
      })
    end
  end
  ...
  context 'target =>' do
    ...
    context 'false' do
      ...
      let(:facts) {{ :pe_concat_basedir => '/tmp' }}
